...
async def cb_vip_crypto(call: CallbackQuery):
    # FIXED: answer() ONCE at the top, then edit message for all states
    await call.answer()
    ...
    uid = call.from_user.id
    # Show "creating" state while we wait for API
...
async def cb_pay_check(call: CallbackQuery):
    ...
    if status == "paid":
        ...
        # Restore original invoice message
        pay_url = f"https://t.me/CryptoBot?start=IV{invoice_id}"
        await call.message.edit_text(
