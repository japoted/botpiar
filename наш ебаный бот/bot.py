...
ready_kb = ReplyKeyboardMarkup(
    keyboard=[
        ...
        [KeyboardButton(text="❌ Отмена")]  # Кнопка отмены для выхода из сессии/поиска
    ],
    resize_keyboard=True,
    is_persistent=True
)
...
async def require_subscription(message: Message) -> bool:
    ...
    if missing:
        await message.answer(
            "⚠️ <b>Для использования бота подпишитесь на наши channels:</b>",
            parse_mode="HTML",
...
async def cmd_start(message: Message, state: FSMContext) -> None:
    ...
    if current_state == VZState.in_session:
        await message.answer(
            ...
            "Сначала завершите её или нажмите «❌ Отмена».",
...
async def cancel_vz_session(message: Message, state: FSMContext) -> None:
    """Хендлер для безопасного выхода из поиска или активной сессии ВЗ"""
    user_id = message.from_user.id
    current_state = await state.get_state()

    # Если юзер на этапе ввода ссылки или в очереди поиска
    if current_state in (VZState.searching, VZState.waiting_for_link):
        if user_id in search_queue:
            try: search_queue.remove(user_id)
            except ValueError: pass
        await state.clear()
        await message.answer("❌ Взаимный пиар отменен. Вы вернулись в меню.", reply_markup=main_kb)
        return

    # Если юзер в сессии с партнером
    if current_state == VZState.in_session:
        ...
        sessions.pop(user_id, None)
        await state.clear()
        await message.answer("❌ Вы отменили сессию взаимной подписки.", reply_markup=main_kb)

        # Сбрасываем партнера, если он есть в сессии
        if partner_id:
            ready_status.discard(partner_id)
            sessions.pop(partner_id, None)
            ...
            await safe_send(
                partner_id, 
                "⚠️ Партнёр отменил сессию взаимного пиара. Вы возвращены в главное меню.", 
                reply_markup=main_kb
            )
        return

    await message.answer("У вас нет активных сессий для отмены.", reply_markup=main_kb)
...
async def start_vz(message: Message, state: FSMContext) -> None:
    ...
    if current_state in (VZState.searching, VZState.in_session):
        await message.answer("⚠️ Вы уже ищете партнера или находитесь в сессии ВЗ!", reply_markup=ready_kb)
        return
    ...
    await message.answer(
        ...
        parse_mode="HTML",
        reply_markup=ready_kb # Сразу даем клавиатуру отмены
    )
...
async def process_link(message: Message, state: FSMContext) -> None:
    ...
    await message.answer("🔍 Ищем вам пару для взаимной подписки...", reply_markup=ready_kb)
...
async def process_ready(message: Message, state: FSMContext) -> None:
    ...
    if partner_id in ready_status: await finish_session(user_id, partner_id)
        ...
        await message.answer("⏳ Вы успешно подтвердили подписку. Ждём партнёра...", reply_markup=ready_kb)
...
async def block_other_messages(message: Message) -> None:
    ...
    await message.answer("⚠️ Вы в сессии ВЗ. Нажмите «✅ Готово» или «❌ Отмена», чтобы выйти.", reply_markup=ready_kb)
...
# ======================================================
# 🔥 ГЛОБАЛЬНЫЙ ОБРАБОТЧИК-ЗАГЛУШКА
# ======================================================
@dp.message()
