...
def get_user(user_id: int) -> tuple[int, int, int, int]:
    cursor.execute("SELECT balance, total_vz, vip_until, is_banned FROM users WHERE user_id = ?", (user_id,))
    res = cursor.fetchone()
    if not res:
...
def set_vip_24h(user_id: int) -> None:
    now = int(time.time())
    _, _, current_vip, _ = get_user(user_id)
...
admin_added_kb = ReplyKeyboardMarkup(
    keyboard=[
        [KeyboardButton(text="✅ Я добавил бота как администратора")],
...
class VZState(StatesGroup):
    waiting_for_admin = State()   # шаг: добавить бота в админы
    waiting_for_link  = State()   # шаг: ввести @канал
    searching         = State()
    in_session        = State()
...
async def check_channel_accessible(chat_id_raw: str) -> bool:
    """Проверяет, доступен ли канал боту."""
    chat_id: str | int = chat_id_raw.strip()
    if str(chat_id).startswith("-100") or str(chat_id).lstrip("-").isdigit():
        try:
            chat_id = int(chat_id)
        except ValueError:
            return False
...
async def check_bot_is_admin(chat_id_raw: str) -> tuple[bool, list[str]]:
    """
    Проверяет, является ли бот администратором с полными правами.
    Возвращает (ok, список_недостающих_прав).
    """
    chat_id: str | int = chat_id_raw.strip()
    if str(chat_id).startswith("-100") or str(chat_id).lstrip("-").isdigit():
        try:
            ...
            return False, ["Неверный ID канала"]
    try:
    ...
    REQUIRED: list[tuple[str, str]] = [
        ("can_manage_chat",       "Управление каналом"),
        ...
        ("can_change_info",       "Изменение данных канала"),
    ]
...
async def finish_session(user_id: int, partner_id: int) -> None:
    ...
    for uid in (user_id, partner_id):
        ...
        await safe_send(uid, "🎉 Взаимная подписка успешно завершена! Вам начислено +500 баллов.", reply_markup=main_kb)
        
        _, _, vip_until, _ = get_user(uid)
...
async def cmd_start(message: Message, state: FSMContext) -> None:
    ...
    _, _, _, is_banned = get_user(user_id)
    if is_banned:
...
async def cancel_vz_session(message: Message, state: FSMContext) -> None:
    ...
    if current_state in (VZState.waiting_for_admin, VZState.searching, VZState.waiting_for_link):
    ...
    if current_state == VZState.in_session:
        ...
        await message.answer(
            ...
            "❌ Причина: Срыв сессии и спам.\n"
            "⏱️ Ограничение на поиск: <b>10 минут</b>.\n"
            "⚠️ Ваш аккаунт помечен как неблагонадежный. Повторные нарушения приведут к перманентному бану.",
        ...
        if partner_id:
            ...
            await safe_send(
                partner_id, 
                "⚠️ Партнёр досрочно разорвал сессию и получил автоматическую блокировку.\n"
...
async def show_profile(message: Message) -> None:
    if not await require_subscription(message): return
    user_id = message.from_user.id
    balance, total_vz, vip_until, is_banned = get_user(user_id)
    ...
    if balance >= 500:
        kb.inline_keyboard.append([InlineKeyboardButton(text="🔄 Обменять 500 баллов на ВЗ+ (24ч)", callback_data="buy_vip_points")])
    ...
    await message.answer(
        ...
        f"<i>💡 За каждое успешное ВЗ начисляется +500 баллов. Накопите 500 баллов и обменяйте их на 24 часа безлимита ВЗ+!</i>",
        parse_mode="HTML", reply_markup=kb if balance >= 500 else main_kb
    )
...
async def show_subscription(message: Message) -> None:
    ...
    await message.answer(
        ...
        "<b>Стоимость:</b> $1.50 USDT или 500 баллов.",
...
async def start_vz(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        f"🤖 <b>Шаг 1 из 2 — Добавьте бота в администраторы</b>\n\n"
        f"Перейдите в настройки вашего канала и добавьте @{bot_username} как <b>администратора со всеми правами</b>:\n\n"
        f"✅ Управление каналом\n"
        f"✅ Публикация сообщений\n"
        f"✅ Редактирование сообщений\n"
        f"✅ Удаление сообщений\n"
        f"✅ Добавление участников\n"
        f"✅ Блокировка участников\n"
        f"✅ Добавление администраторов\n"
        f"✅ Изменение данных канала\n\n"
        f"После добавления нажмите кнопку ниже 👇",
        parse_mode="HTML", reply_markup=admin_added_kb
    )
    await state.set_state(VZState.waiting_for_admin)


@dp.message(VZState.waiting_for_admin, F.text == "✅ Я добавил бота как администратора")
async def admin_added_confirm(message: Message, state: FSMContext) -> None:
    await message.answer(
        "📢 <b>Шаг 2 из 2 — Укажите ваш канал</b>\n\n"
        "Отправьте @username или ID вашего канала:\n\n"
        "📢 Публичный: <code>@channel</code> или <code>t.me/channel</code>\n"
        "🔒 Приватный: <code>-100XXXXXXXXXX::https://t.me/+invite</code>",
        parse_mode="HTML", reply_markup=ready_kb
    )
    await state.set_state(VZState.waiting_for_link)


@dp.message(VZState.waiting_for_admin)
async def waiting_admin_other(message: Message) -> None:
    raw = message.text.strip() if message.text else ""
    if raw in ("❌ Отмена", "Начать вз", "Профиль", "Подписка"):
        return  # обработается соответствующим хендлером
    bot_username = await get_bot_username()
    await message.answer(
        f"⚠️ Сначала добавьте @{bot_username} как администратора в ваш канал,\n"
        f"затем нажмите «✅ Я добавил бота как администратора».",
        reply_markup=admin_added_kb
    )
...
async def process_link(message: Message, state: FSMContext) -> None:
    ...
    BUTTON_TEXTS = {
        "✅ Готово", "❌ Отмена", "Начать вз", "Профиль", "Подписка",
        "✅ Я добавил бота как администратора"
    }
    if not raw_text or raw_text.startswith("/") or raw_text in BUTTON_TEXTS:
        await message.answer(
            "⚠️ Отправьте @username или ID вашего канала.\n\n"
            "📢 Публичный: <code>@channel</code> или <code>t.me/channel</code>\n"
            "🔒 Приватный: <code>-100XXXXXXXXXX::https://t.me/+invite</code>",
            parse_mode="HTML"
        )
    ...
    if "::" in raw_text:
        ...
        if not (chat_id.startswith("-100") or chat_id.isdigit()) or not invite_link.startswith("http"):
            await message.answer("⚠️ Ошибка формата! Используйте шаблон: <code>-100ID::https://t.me/+invite</code>", parse_mode="HTML")
            return
        target_id = chat_id
        ...
        target_id = extracted
        display_link = f"https://t.me/{extracted.lstrip('@')}"

    # Проверка: канал доступен
    if not await check_channel_accessible(target_id):
        set_penalty(user_id, 600)
        await state.clear()
        await message.answer(
            "❌ <b>Канал недоступен или не существует!</b>\n\n"
            "Убедитесь, что канал существует и бот добавлен в него.\n\n"
            "⏱️ За отправку недействительной ссылки вы получили ограничение на <b>10 минут</b>.",
            parse_mode="HTML", reply_markup=main_kb
        )
        return

    # Проверка: бот является администратором со всеми правами
    ok, missing = await check_bot_is_admin(target_id)
    if not ok:
        bot_username = await get_bot_username()
        if missing == ["Бот не найден в канале"] or missing == ["Бот не является администратором"]:
            await message.answer(
                f"⚠️ <b>Бот не является администратором канала!</b>\n\n"
                f"Добавьте @{bot_username} в администраторы вашего канала и попробуйте снова.",
                parse_mode="HTML", reply_markup=admin_added_kb
            )
            await state.set_state(VZState.waiting_for_admin)
        else:
            ...
            await message.answer(
                f"⚠️ <b>Боту не хватает прав в канале!</b>\n\n"
                f"Выдайте @{bot_username} следующие права:\n{missing_text}\n\n"
                f"После этого снова отправьте @username канала.",
                parse_mode="HTML"
            )
        return

    user_links[user_id] = target_id
    display_links[user_id] = display_link

    await message.answer("🔍 Подбираем для вас подходящего партнера...", reply_markup=ready_kb)
...
async def check_queue() -> None:
    while len(search_queue) >= 2:
        ...
        for u, partner in ((user1, user2), (user2, user1)):
            ...
            await safe_send(
                u, f"✅ <b>Партнер найден!</b>\n\n👉 Ссылка для подписки: {display_links.get(partner, '—')}\n\nПосле выполнения подписки нажмите кнопку.",
                parse_mode="HTML", reply_markup=ready_kb
            )


@dp.message(VZState.searching, F.text == "✅ Готово")
async def ready_while_searching(message: Message) -> None:
    await message.answer(
        "⏳ Ищем партнёра, подождите...\nНажмите «❌ Отмена», чтобы выйти из очереди.",
...
async def process_ready(message: Message, state: FSMContext) -> None:
    ...
    if partner_id in ready_status: await finish_session(user_id, partner_id)
        ...
        await safe_send(partner_id, "🔔 Партнер выполнил условия подписки и ожидает вас!")
