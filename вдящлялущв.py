...
async def check_bot_is_admin(chat_id_raw: str) -> bool:
...
async def show_profile(message: Message) -> None:
    ...
    await message.answer(
        ...
        f"<i>💡 За каждое успешное ВЗ начисляется +500 баллов. Накопите 5000 баллов (10 ВЗ) и обменяйте их на 24 часа безлимита ВЗ+!</i>",
...
async def start_vz(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        "🔗 <b>Пришлите ссылку на продвигаемый канал.</b>\n\n"
        "📢 Публичный формат: <code>t.me/link</code>\n"
        "🔒 Приватный формат: <code>ID::ССЫЛКА</code>",
...
async def process_link(message: Message, state: FSMContext) -> None:
    ...
    # Игнорируем нажатия на кнопки клавиатуры — исправление бага "✅ Готово" как ссылка
    BUTTON_TEXTS = {"✅ Готово", "❌ Отмена", "Начать вз", "Профиль", "Подписка"}
    if not raw_text or raw_text.startswith("/") or raw_text in BUTTON_TEXTS:
        await message.answer(
            "⚠️ Отправьте корректную ссылку на ваш ресурс.\n\n"
            "📢 Публичный: <code>t.me/link</code>\n"
            "🔒 Приватный: <code>ID::ССЫЛКА</code>",
            parse_mode="HTML"
        )
        return
    ...
    if "::" in raw_text:
        ...
        if not (chat_id.startswith("-100") or chat_id.isdigit()) or not invite_link.startswith("http"):
            await message.answer("⚠️ Ошибка формата! Используйте шаблон: <code>ID::ССЫЛКА</code>", parse_mode="HTML")
            return
        target_id = chat_id
    ...
    # Проверка: доступен ли канал
    if not await check_channel_accessible(target_id):
        set_penalty(user_id, 600)
        await state.clear()
        await message.answer(
            "❌ <b>Канал недоступен или не существует!</b>\n\n"
            "Убедитесь, что ссылка верна и бот добавлен в канал как администратор.\n\n"
            "⏱️ За отправку недействительной ссылки вы получили ограничение на <b>10 минут</b>.",
            parse_mode="HTML", reply_markup=main_kb
        )
        return

    # Проверка: бот является администратором канала
    if not await check_bot_is_admin(target_id):
        await message.answer(
            "⚠️ <b>Бот не является администратором вашего канала!</b>\n\n"
            "Добавьте бота как администратора с правом просмотра участников и попробуйте снова.",
            parse_mode="HTML"
        )
        return
